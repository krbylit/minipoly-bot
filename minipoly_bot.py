...
"""Discord bot implementation of the Minipoly game.
"""
...
__author__ = "Kirby Little"
__email__ = "krbylit@gmail.com"
...
@bot.command(
    help="Starts a game of Minipoly with 24 spaces and a starting GO space. "
    "Rent "
...
@bot.command(
    help="First rolls a die for your movement. Then moves that many spaces "
    "for you. "
    "Movement automatically checks if your new space has an owner, "
    "and handles "
    "your payment of rent to them. Your new current space's info is then "
    "printed, as well as a notification of rent payment if that "
    "happens. Finally "
    "game status is checked, and a winner is declared if there is one.",
    brief="Rolls a die and moves you, paying rent on new space if necessary.",
    name="roll",
    # optional shorter command call, prefix w/ $ in chat to execute
)
async def roll(ctx):
    """Rolls a die for movement, moves the player and does rent payment
    handling,
    ...
    if money_after_move == 0:
        await ctx.channel.send(f"Ope. {user_name} is out of money :(\nGGWP {user_name}")
...
@bot.command(
    help="Looks at your current space and prints its information: board "
    "position, "
