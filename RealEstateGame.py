# Author: Kirby Little
# GitHub username: krbylit
# Date:05/25/2022
# Description: A simplified version of the 'Monopoly' board game.


class RealEstateGame:
    """Represents the Real Estate board game. Controls board creation, player
    creation, movement, Space purchasing, and game-state check. Communicates with
    Player class to move, purchase Spaces, and pay rent. Communicates with Space
    class to set ownership."""

    def __init__(self):
        """Initializes the Real Estate game with dictionaries to store players and
        game board spaces. Spaces dictionary is keyed with each Space's name with
        Space objects as values. Players dictionary is keyed with each Player's name
    ...
    def create_spaces(self, go_money: int, rent_values: list):
        ...
        one Space per value. Buy prices are generated depending on the rent values.
        Receives value of money awarded when passing GO and list of rent values."""
    ...
    def create_player(self, player_name: str, go_money: int):
        """Creates a Player object and adds it to the "players" dictionary. Receives a
        ...
        self._players[player_name]: Player = Player(
            self._spaces["GO"], go_money, True, 0, spaces, player_name
        )

    def get_player_account_balance(self, player_name: str):
        """Returns input player's account balance by running Player's
        get_money_balance method. Receives player name to access "players" dictionary
        for Player object."""
        return self._players[player_name].get_money_balance()

    def get_player_current_position(self, player_name: str):
        """Returns input player's current position as integer by running Player's
        get_position method. Receives player name to access "players" dictionary for
        Player object."""
    ...
    def buy_space(self, player_name: str):
        """Purchases the input player's current Space for that player. Uses Player's
        methods get_current_space and set_money_balance to get the Space object and
        adjust player's money. Uses Space's methods get_buy_value to get purchase
        price, get_position to make sure it's not the GO Space, get_owner to check if
        its owned already, and set_owner to set the Space's owner to the purchasing
        player. Uses RealEstateGame's get_player_account_balance to get player's
        balance. Receives player's name to access "players" dictionary for the Player
        object."""
    ...
    def move_player(self, player_name: str, move_spaces: int):
        ...
        not, returns without action. Uses RealEstateGames' get_player_account_balance
        to get player's balance. Uses Player's methods set_position to handle
        moving, get_current_space to get the Space object the player moved to, and
        set_money_balance to adjust balances for rent. Uses Space's methods get_owner
        to get the Space's owner's Player object and get_rent_value to get rent price.
        Receives player's name to access "players" dictionary for Player object and
    ...
    def check_game_over(self):
        """Checks game state to see if it is finished. Iterates through "players"
        dictionary to check each player's status. Uses Player's methods get_status to
...
class Player:
    ...
    active status, board position, dictionary of board spaces, and GO award money
    attributes. Communicates with RealEstateGame class to move, purchase Spaces,
    pay rent, and set active status. Communicates with Space class to set ownership
    and pay rent."""
    ...
    def __init__(
        ...
        active status, name, dictionary of board spaces, and position parameters.
        Active status attribute is set by init."""
        self._current_space: Space = current_space
        self._money_balance: int = go_money
    ...
    def set_money_balance(self, value: int):
        """Adjusts the player's account balance and returns the absolute value of the
        amount adjusted. Cannot be adjusted below 0. Sets active status to False if
    ...
    def set_position(self, move_spaces: int):
        """Increments player's current position by given integer amount and sets
        their current space to the Space object at that board index. Wraps back
        around GO space and adds GO money if player passes end of the board. Uses
        Player's methods get_position to get position and set_money_balance to award GO
...
class Space:
    """Represents a board space object with position, buy and rent values, and owner
    ...
    def __init__(
        ...
        """Initializes Space object with position, rent and buy values, and owner
        attributes."""
